...
(() => {
        ...
        return new Promise((resolve, reject) => {
            const onReady = () => {
                try {
                    ...
                    // Split-view synchronised scrolling (editor ↔ preview).
                    // Mounted here (not in init()) because the editor
                    // object doesn't exist until after Monaco loads.
                    initScrollSync();
    ...
    // ============================================================
    //
    // In split view, the user can scroll the editor and the preview
    // independently. That's annoying when you want to track "as I
    // scroll, where am I in the rendered output?" — so we wire up a
    // proportional sync: when one side scrolls, we set the other side's
    // scrollTop to (maxScroll × ratio).
    //
    // Implementation notes:
    // - Monaco exposes scroll changes via `editor.onDidScrollChange`,
    //   which also tells us whether scrollTop actually changed (vs.
    //   scroll width / height only).
    // - The preview pane scrolls like a normal overflow:auto div.
    // - We use a `_scrollSyncing` flag to break the feedback loop:
    //   when side A is the source, we set the flag, programmatically
    //   scroll side B, and side B's handler bails out because the
    //   flag is on.
    // - When one pane is hidden (edit-only / preview-only mode) we
    //   still attach the listener but the visible side has nothing
    //   to sync to — sync is a no-op.
    // - Inaccurate by design: the editor and the rendered HTML have
    //   different line heights / paragraph heights, so a proportional
    //   sync is approximate. Element-based mapping (walk the DOM and
    //   match heading↔heading, paragraph↔paragraph) is more accurate
    //   but a lot of code; the user said "scrolling together" is
    //   enough, so proportional it is.
    let _scrollSyncing = false;

    function getMonacoScrollInfo() {
        // Monaco's own scroll position is `editor.getScrollTop()`;
        // the total scrollable height is `editor.getScrollHeight()`
        // and the visible viewport height is `editor.getLayoutInfo().height`.
    ...
    function getPreviewScrollInfo() {
        const target = els2.previewContent;
        if (!target) return { scrollTop: 0, max: 0 };
        return {
    ...
    function syncEditorToPreview() {
        ...
        const preview = els2.previewContent;
        if (!preview || preview.clientHeight === 0) return;  // hidden (edit-only mode)
        const m = getMonacoScrollInfo();
        if (m.max === 0) return;  // content fits in viewport — nothing to sync
        const ratio = m.scrollTop / m.max;
        _scrollSyncing = true;
        try {
            const p = getPreviewScrollInfo();
            preview.scrollTop = p.max * ratio;
        } finally {
    ...
    function syncPreviewToEditor() {
        ...
        const p = getPreviewScrollInfo();
        if (p.max === 0) return;
        const ratio = p.scrollTop / p.max;
        _scrollSyncing = true;
        try {
            editor.setScrollTop(m.max * ratio);
        } finally {
            // The setScrollTop call may not fire the scroll event
            // synchronously, but be safe: clear the flag on the next
            // animation frame so the editor's scroll handler (if it
            // fires) bails out.
    ...
    function init() {
        ...
        // Note: Preview / Deploy buttons and the script-output modal
        // are no longer wired here — they live in _Layout.cshtml
        // (top nav) and are driven by script-runner.js, which runs
        // on every page, not just the editor.

        // initScrollSync() is mounted in mountMonaco's onReady
        // because the Monaco editor doesn't exist at IIFE-init time.
    }
    init();
